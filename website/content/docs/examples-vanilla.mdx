...
import { BasicFadeInVanilla, CharacterRevealVanilla, LineByLineVanilla, AutoRevertVanilla, ResponsiveSplitVanilla, WithGSAPVanilla } from "../../app/components/examples";
...
Pass an animation promise to `revertOnComplete` to automatically restore the original HTML when the animation finishes. This cleans up the split spans and returns the text to its natural state.
...
// Revert to original HTML when animation completes
splitText(element, {
